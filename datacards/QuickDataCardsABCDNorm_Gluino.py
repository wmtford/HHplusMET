...
if run2D:
	...
	f2=TFile("/eos/uscms/store/user/emacdona/boostedHiggsPlusMET/boosted_2DT5HH_noVeto/ALPHABET_2DSignal.root", "READ")
	...
	f2=TFile("/eos/uscms/store/user/emacdona/boostedHiggsPlusMET/T5HH1D_FullSIM/ALPHABET_1DT5HH_FullSIM.root", "READ")
	if runVeto==True: f2Veto=TFile("/eos/uscms/store/user/emacdona/boostedHiggsPlusMET/T5HH1D_FullSIM_veto/ALPHABET_1DSignal.root", "READ")
...
if runVeto==True:
	...
	for line in fcard:
		if "observation" in line:
			...
			else: newline=newline +"%.4f 1.0 %.4f 1.0 \n" %(SignalB2Veto.Integral(1,5), SignalB1Veto.Integral(1,5))
			fcardout.write(newline);
		elif "Beta_2H rateParam" in line:
	...
	for line in fcard:
		if "observation" in line:
			...
			else: newline=newline +"%.4f 1.0 %.4f 1.0 %.4f 1.0 \n" %(SignalMETVeto.GetBinContent(1),SignalMETVeto.GetBinContent(2),SignalMETVeto.GetBinContent(3))
			fcardout.write(newline);
		elif "Epsilon1 rateParam" in line:
...
if runVeto==True:
	...
	# os.system("combine -M GoodnessOfFit -n %s%s%d_LSP%d_%s_Combo %s%s%d_LSP%d_%s_Combo.txt  --algo=saturated --toysFreq -t 500 -s %i" %(whichDString,signal,hino,LSP,dataMCString,whichDString,signal,hino,LSP,dataMCString, seed))
