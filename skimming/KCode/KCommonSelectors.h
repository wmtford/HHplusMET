		...
		virtual void GetResult() {
			...
			for(unsigned j = 0; j < looper->Jets->size(); ++j){
				...
				if(Jet.Pt()>30 and abs(Jet.Eta())<5.0 and abs(Jet.Eta())>2.4){
					double dphi = abs(KMath::DeltaPhi(Jet.Phi(),((doMET) ? looper->METPhi : looper->MHTPhi)));
...
class KHEMDPhiVetoSelector : public KFilterSelector {
	public:
		...
		bool InWideHEMRegion(const TLorentzVector& tlv, double ptcut, double MHTPhi){
			return tlv.Pt()>ptcut and -3.2 < tlv.Eta() and tlv.Eta() < -1.2 and -1.77 < tlv.Phi() and tlv.Phi() < -0.67 and KMath::DeltaPhi(tlv.Phi(),((doMET) ? looper->METPhi : looper->MHTPhi))<0.5;
		}
		virtual void GetResult() {
			bool activity = false;
			//check electrons - only isolated ones
			...
			//check jets
			for(const auto& Jet : *looper->Jets){
				if(!activity and InWideHEMRegion(Jet,minpt,((doMET) ? looper->METPhi : looper->MHTPhi))) activity = true;
				if(activity) break;
			}
			result = !activity;
		}
