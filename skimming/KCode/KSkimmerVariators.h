...
//----------------------------------------------------
//variation of jet-related variables for Zinv cleaning
class KCleanAK4Variator : public KVariator {
	public:
		//constructor
		KCleanAK4Variator() : KVariator() { }
		KCleanAK4Variator(string name_, OptionMap* localOpt_) : KVariator(name_,localOpt_)
		{
			//check options
		}
...
REGISTER_VARIATOR(CleanAK4);

//----------------------------------------------------
//variation of jet-related variables for Zinv cleaning
class KPoorCleanVariator : public KVariator {
	public:

		//constructor
		KPoorCleanVariator() : KVariator(),
			JetsAK8(new vector<TLorentzVector>),
			JetsAK8_softDropMass(new vector<double>),
			JetsAK8_pfMassIndependentDeepDoubleBvLJetTagsProbHbb(new vector<double>),
			JetsAK8_doubleBDiscriminator(new vector<double>),
			JetsAK8_ID(new vector<bool>),
			JetsAK8_subjets(new vector< vector <TLorentzVector> >)
		{
			clear();
		}
		KPoorCleanVariator(string name_, OptionMap* localOpt_) : KVariator(name_,localOpt_),
			JetsAK8(new vector<TLorentzVector>),
			JetsAK8_softDropMass(new vector<double>),
			JetsAK8_pfMassIndependentDeepDoubleBvLJetTagsProbHbb(new vector<double>),
			JetsAK8_doubleBDiscriminator(new vector<double>),
		...
		~KPoorCleanVariator() {
			delete JetsAK8;
			delete JetsAK8_softDropMass;
			delete JetsAK8_pfMassIndependentDeepDoubleBvLJetTagsProbHbb;
			delete JetsAK8_doubleBDiscriminator;
			delete JetsAK8_ID;
			delete JetsAK8_subjets;
		}

		//constructor
		// KPoorCleanVariator() : KVariator() { }
		// KPoorCleanVariator(string name_, OptionMap* localOpt_) : KVariator(name_,localOpt_)//,
		// JetsAK8_new(new vector<TLorentzVector>),
		// JetsAK8_softDropMass_new(new vector<double>),
		// JetsAK8_pfMassIndependentDeepDoubleBvLJetTagsProbHbb_new(new vector<double>),
		// JetsAK8_doubleBDiscriminator_new(new vector<double>),
		// JetsAK8_ID_new(new vector<bool>),
		// JetsAK8_subjets_new(new vector< vector <TLorentzVector> >)
		// {
		// 	//check options
		// 	clear();
		// }
		...
		// ~KPoorCleanVariator() {
		// 	delete JetsAK8_new;
		// 	delete JetsAK8_softDropMass_new;
		// 	delete JetsAK8_pfMassIndependentDeepDoubleBvLJetTagsProbHbb_new;
		// 	delete JetsAK8_doubleBDiscriminator_new;
		// 	delete JetsAK8_ID_new;
		// 	delete JetsAK8_subjets_new;
		// }
		...
		virtual void ListBranches(){
			//set up linked branches for clean variation
			linkbranches = {
				//Do poor-man's cleaning
				new KLinkedBranchVL(KBranchVL(&looper->JetsAK8,"JetsAK8"),KBranchVL(&JetsAK8)),
				new KLinkedBranchVD(KBranchVD(&looper->JetsAK8_softDropMass,"JetsAK8_softDropMass"),KBranchVD(&JetsAK8_softDropMass)),
				new KLinkedBranchVD(KBranchVD(&looper->JetsAK8_pfMassIndependentDeepDoubleBvLJetTagsProbHbb,"JetsAK8_pfMassIndependentDeepDoubleBvLJetTagsProbHbb"),KBranchVD(&JetsAK8_pfMassIndependentDeepDoubleBvLJetTagsProbHbb)),
				new KLinkedBranchVD(KBranchVD(&looper->JetsAK8_doubleBDiscriminator,"JetsAK8_doubleBDiscriminator"),KBranchVD(&JetsAK8_doubleBDiscriminator)),
				new KLinkedBranchVB(KBranchVB(&looper->JetsAK8_ID,"JetsAK8_ID"),KBranchVB(&JetsAK8_ID)),
				new KLinkedBranchVVL(KBranchVVL(&looper->JetsAK8_subjets,"JetsAK8_subjets"),KBranchVVL(&JetsAK8_subjets))
			};
		}
		...
		virtual void DoVariation() {
			...
			// if (looper->JetsAK8->size()>0) std::cout<<"Thank God, we have something..."<<std::endl;
			...
			clear();
			int nPhotons=0;
			vector<int> Photon_idx;
			Photon_idx.resize(0);
			for(unsigned int i=0;i<looper->Photons->size();i++){
				if (looper->Photons->at(i).Pt()>100 && looper->Photons_fullID->at(i) && (!(looper->Photons_hasPixelSeed->at(i)) ) ){
					Photon_idx.push_back(i);
					nPhotons++;
				}
			}

			if (nPhotons>0){
				const auto& firstPhoton = looper->Photons->at(Photon_idx[0]);

				//loop through jets and do poor-man's cleaning myself
				bool photonFound = false;
				for(unsigned int j = 0; j<looper->JetsAK8->size(); j++){
					//if jet matches to highest-pt photon, remove jet
					const auto& Jet = looper->JetsAK8->at(j);
					if (!photonFound && Jet.DeltaR(firstPhoton)<0.8) {
						photonFound=true;
						continue; //we have found our photon!
					}

					JetsAK8->push_back(Jet);
					JetsAK8_softDropMass->push_back(looper->JetsAK8_softDropMass->at(j));
					JetsAK8_pfMassIndependentDeepDoubleBvLJetTagsProbHbb->push_back(looper->JetsAK8_pfMassIndependentDeepDoubleBvLJetTagsProbHbb->at(j));
					JetsAK8_doubleBDiscriminator->push_back(looper->JetsAK8_doubleBDiscriminator->at(j));
					JetsAK8_ID->push_back(looper->JetsAK8_ID->at(j));
					JetsAK8_subjets->push_back(looper->JetsAK8_subjets->at(j));
				}
			}
			else { //no photons found, so don't do any cleaning
				*JetsAK8 = *looper->JetsAK8;
				*JetsAK8_softDropMass = *looper->JetsAK8_softDropMass;
				*JetsAK8_pfMassIndependentDeepDoubleBvLJetTagsProbHbb = *looper->JetsAK8_pfMassIndependentDeepDoubleBvLJetTagsProbHbb;
				*JetsAK8_doubleBDiscriminator = *looper->JetsAK8_doubleBDiscriminator;
				*JetsAK8_ID = *looper->JetsAK8_ID;
				*JetsAK8_subjets = *looper->JetsAK8_subjets;
			}
		...
		void clear(){
			JetsAK8->clear();
			JetsAK8_softDropMass->clear();
			JetsAK8_pfMassIndependentDeepDoubleBvLJetTagsProbHbb->clear();
			JetsAK8_doubleBDiscriminator->clear();
			JetsAK8_ID->clear();
			JetsAK8_subjets->clear();
		}
...
//Emily's attempt to implement the resolved isoTrack definition
class KResIsoTrackVariator : public KVariator {
	public:
		//constructor
		KResIsoTrackVariator() : KVariator() { }
		KResIsoTrackVariator(string name_, OptionMap* localOpt_) : KVariator(name_,localOpt_)
		{
			clear();
		}
		// ~KResIsoTrackVariator() { }
		virtual void CheckBranches(){
			ListBranches();
			//only check branch0 since branch1 does not come from tree
			for(auto& branch : linkbranches){
		...
		virtual void ListBranches(){
			linkbranches = {
				//scalars get recalculated and replaced
				new KLinkedBranchI(KBranchI(&looper->isoMuonTracks,"isoMuonTracks"),KBranchI(&isoMuonTracks)),
				new KLinkedBranchI(KBranchI(&looper->isoElectronTracks,"isoElectronTracks"),KBranchI(&isoElectronTracks)),
				new KLinkedBranchI(KBranchI(&looper->isoPionTracks,"isoPionTracks"),KBranchI(&isoPionTracks)),
			};
		}
		virtual void DoVariation() {
			for(auto& branch : linkbranches){
			...
			for(unsigned im = 0; im < looper->TAPMuonTracks->size(); ++im){  //dz and pdgid cuts in place at ntuple creation
				if (looper->TAPMuonTracks->at(im).Pt()<=5.0 || fabs(looper->TAPMuonTracks->at(im).Eta())>2.5 || looper->TAPMuonTracks_dxypv->at(im)>=0.2 || looper->TAPMuonTracks_mT->at(im)>100.0) continue;
				bool isIso = false;
				isIso = (
					(looper->TAPMuonTracks->at(im).Pt()<25.0 && looper->TAPMuonTracks_pfRelIso03chg->at(im)*looper->TAPMuonTracks->at(im).Pt() < 5.0) ||
					(looper->TAPMuonTracks_pfRelIso03chg->at(im)<0.2)
				);
				if (isIso) isoMuonCounter++;
			}

			//Then electrons
			for(unsigned im = 0; im < looper->TAPElectronTracks->size(); ++im){  //dz and pdgid cuts in place at ntuple creation
				if (looper->TAPElectronTracks->at(im).Pt()<=5.0 || fabs(looper->TAPElectronTracks->at(im).Eta())>2.5 || looper->TAPElectronTracks_dxypv->at(im)>=0.2 || looper->TAPElectronTracks_mT->at(im)>100.0) continue;
				bool isIso =  false;
				isIso = (
					(looper->TAPElectronTracks->at(im).Pt()<25.0 && looper->TAPElectronTracks_pfRelIso03chg->at(im)*looper->TAPElectronTracks->at(im).Pt() < 5.0) ||
					(looper->TAPElectronTracks_pfRelIso03chg->at(im)<0.2)
				);
				if (isIso) isoElectronCounter++;
			}

			//Lastly pions
			for(unsigned im = 0; im < looper->TAPPionTracks->size(); ++im){  //dz and pdgid cuts in place at ntuple creation
				if (looper->TAPPionTracks->at(im).Pt()<=10.0 || fabs(looper->TAPPionTracks->at(im).Eta())>2.5 || looper->TAPPionTracks_dxypv->at(im)>=0.2 || looper->TAPPionTracks_mT->at(im)>100.0) continue;
				if (looper->TAPPionTracks_leptonMatch->at(im)!=0) continue;
				bool isIso =  false;
				isIso = (
					(looper->TAPPionTracks->at(im).Pt()<25.0 && looper->TAPPionTracks_pfRelIso03chg->at(im)*looper->TAPPionTracks->at(im).Pt() < 5.0) ||
					(looper->TAPPionTracks_pfRelIso03chg->at(im)<0.1)
				);
				if (isIso) isoPionCounter++;
			}


			isoMuonTracks = isoMuonCounter;
			isoElectronTracks = isoElectronCounter;
			isoPionTracks = isoPionCounter;
...
REGISTER_VARIATOR(ResIsoTrack);


class KDeltaPhiMETVariator : public KVariator {
	public:
		//constructor
		KDeltaPhiMETVariator() : KVariator() { }
		KDeltaPhiMETVariator(string name_, OptionMap* localOpt_) : KVariator(name_,localOpt_)
		{
			clear();
		}
		...
		virtual void DoVariation() {
			...
			for(const auto& Jet : *looper->Jets){
				if(Jet.Pt()>30 && abs(Jet.Eta())<5.0){
					dphi[counter] = abs(KMath::DeltaPhi(Jet.Phi(),looper->METPhi));
					++counter;
				}
				if(counter>=4) break;
			}
