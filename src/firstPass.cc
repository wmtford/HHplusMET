...
int main(int argc, char** argv){
	...
	for( int iSample = 0 ; iSample < skims.ntuples.size() ; iSample++){
		...
		double HiggsCan_AvgMass_4b=0;
		double HiggsCan_MassDiff_4b=0;
		...
		TBranch *b_HiggsCan_MassDiff_4b, *b_HiggsCan_AvgMass_4b;
		...
		newtree->Branch("HiggsCan_AvgMass_4b", &HiggsCan_AvgMass_4b, "HiggsCan_AvgMass_4b/D");
		newtree->Branch("HiggsCan_MassDiff_4b", &HiggsCan_MassDiff_4b, "HiggsCan_MassDiff_4b/D");
		...
		newtree->SetBranchAddress("HiggsCan_AvgMass_4b", &HiggsCan_AvgMass_4b, &b_HiggsCan_AvgMass_4b);
		newtree->SetBranchAddress("HiggsCan_MassDiff_4b", &HiggsCan_MassDiff_4b, &b_HiggsCan_MassDiff_4b);
		...
		for( int iEvt = 0 ; iEvt < numEvents ; iEvt++ ){
			...
			nNotIsoResolvedH=0;
			double test_boostH_mass = -999; //For cases where there's only one boosted Higgs
			...
			for(unsigned int fj=0; fj<ntuple->JetsAK8->size();++fj){
				...
				if(ntuple->JetsAK8->at(fj).Pt()>300) {
					if (ntuple->JetsAK8_doubleBDiscriminator->at(fj)>bbtagCut) test_boostedMass.push_back(ntuple->JetsAK8_prunedMass->at(fj));
					if (ntuple->JetsAK8_prunedMass->at(fj)<85.0 || ntuple->JetsAK8_prunedMass->at(fj)>135.0) continue;
			...
			//based on Moriond Recommendations for comb CSV: https://twiki.cern.ch/twiki/bin/view/CMS/BtagRecommendation80XReReco#Recommendation_for_combining_b_a
			...
			for(unsigned int j=0; j<ntuple->Jets->size();++j){
				...
				if(!Overlap)IsoAK4Jets.push_back(j);
			...
			if (BTags>1 && JetsBCan.size() >=4){ //Find the four jets with the highest b-discriminator
				...
				if (MassDiff1<MassDiff2 && MassDiff1<MassDiff3){
					...
					if (BTags>3){
						HiggsCan_MassDiff_4b = MassDiff1;
						HiggsCan_AvgMass_4b = MassAvg1;
					}
					...
					deltaR_max = max(deltaR1, deltaR2);
					if (BTags>3){
						HiggsCan_MassDiff_4b = MassDiff2;
						HiggsCan_AvgMass_4b = MassAvg2;
					}
					...
					deltaR_max = max(deltaR1, deltaR2);
					if (BTags>3){
						HiggsCan_MassDiff_4b = MassDiff3;
						HiggsCan_AvgMass_4b = MassAvg3;
					}
